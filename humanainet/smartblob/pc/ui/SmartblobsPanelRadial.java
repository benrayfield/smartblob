...
import humanainet.smartblob.core.radial_old.transitioningToRadialStillUsingTriangleMesh.RadialMeshUtil;
...
@Deprecated //Radialsmartblob extends LayeredZigzag so both use the same display (SmartblobsPanelAdvancedCurve)
public class SmartblobsPanelRadial extends JPanel implements MouseMotionListener, MouseListener, Task{
	...
	public float maxSecondsToSimAtOnce = .2f;
	
	public long frames;
	
	protected boolean drawLines = true;
	
	protected boolean ifDrawLinesThenAsBrightnessOfLineMuscle = true;
	
	protected boolean drawTestOfRadialZigzag = false;
	...
	public SmartblobsPanelRadial(){
		this(RadialMeshUtil.newSimWithDefaultOptions());
		//LayeredZigzag y = SmartblobUtil.simpleSmartblobExample();
		/*
		LayeredZigzag y = RadialMeshUtil.smartblobExample();
		synchronized(sim.smartblobs){
			sim.smartblobs.add(y);
		}
		//LayeredZigzag z = SmartblobUtil.simpleSmartblobExample();
		LayeredZigzag z = RadialMeshUtil.smartblobExample();
		...
		//LayeredZigzag x = SmartblobUtil.simpleSmartblobExample();
		LayeredZigzag x = RadialMeshUtil.smartblobExample();
		...
		for(int i=0; i<10; i++){
			LayeredZigzag blob = RadialMeshUtil.smartblobExample();
			for(MovCorner c : blob.corners()){
				c.x += 100*(i%5);
				if(i >= 5) c.y += 200;
			}
			synchronized(sim.smartblobs){
				sim.smartblobs.add(blob);
			}
		}
	...
	/** Starts self as Task. */
	public SmartblobsPanelRadial(SmartblobSim sim){
		this.sim = sim;
