...
import humanainet.smartblob.core.brain.brains.FreqsBrain;
import humanainet.smartblob.core.brain.brains.NFreqRotater;
import humanainet.smartblob.core.brain.brains.RandBrain;
...
public class CurvblobUtil{
	private CurvblobUtil(){}
	...
	public static List<ChangeSpeed> defaultChangeSpeeds = Collections.unmodifiableList(Arrays.<ChangeSpeed>asList(
		...
		//new MusclesFeelAndReact(2000, Rand.strongRand)
		//new MusclesFeelAndReact(35000, Rand.strongRand)
		//new MusclesFeelAndReact(50000, Rand.strongRand)
		new MusclesFeelAndReact(60000, Rand.strongRand)
		//new MusclesFeelAndReact(80000, Rand.strongRand)
			
		//new HoldSpeedConstant(40))
	));
	
	//"TODO change targetDistance instead of MusclesFeelAndReact being a ChangeSpeed"
	...
	public static LayeredZigzag simpleSmartblobExample(){
		...
		LineMuscleFactory muscleFactory = new LineMuscleFactory();
		//Brain brain = new RandBrain(layerSize);
		//Brain brain = new RandBrain(muscleFactory.maxMuscles(layers, layerSize));
		Brain brain = new NFreqRotater(muscleFactory.maxMuscles(layers, layerSize), new int[]{3}, new double[]{0});
		//Brain brain = new FreqsBrain();
		...
		//LayeredZigzag blob = new LayeredZigzag(null, new LineMuscleFactory(), 3, 24, 100, 100, 40);
		
		LayeredZigzag blob = new LayeredZigzag(0, false, brain, layers, layerSize, 100, 100, 60);
	...
	public static LayeredZigzag wavegear(
		...
		LayeredZigzag blob = new LayeredZigzag(0, false, brain, layers, layerSize, 0, 0, 0);
		MuscleFactory m = new LineMuscleFactory();
		blob.mutableMuscles().addAll(Arrays.asList( m.newMuscles(blob, m.maxMuscles(blob)) ));
	...
	*/
	public static float distanceToOuterLineSegment(float y, float x, MovTri t){
		MovCorner cornerA = t.adjacentCorners.get(0), cornerB = t.adjacentCorners.get(1);
		float ay = cornerA.y;
		float ax = cornerA.x;
		float by = cornerB.y;
		float bx = cornerB.x;
	...
	*/
	public static boolean isInsideBorder(float y, float x, MovTri closestOuterTri){
		MovCorner cornerA = closestOuterTri.adjacentCorners.get(0), cornerB = closestOuterTri.adjacentCorners.get(1);
		/*if(cornerA.corner.point == closestOuterTri.smartblob.layerSize()-1){ //wrap index
			//TODO is this needed? It seemed to work with plain LayeredZigzag (curvblob) wherever it hit
			MovCorner temp = cornerA;
			cornerA = cornerB;
			cornerB = temp;
		}*/
		float y1 = cornerA.y;
		...
		float y2 = cornerB.y;
		float x2 = cornerB.x;
		MovCorner p2 = closestOuterTri.adjacentCorners.get(2);
		float ay = y, ax = x;
	...
	public static void getClosestPointToInfiniteLine(float getYX[], MovTri borderTri, float y, float x){
		MovCorner c0 = borderTri.adjacentCorners.get(0), c1 = borderTri.adjacentCorners.get(1);
	...
	*/
	...
	public static float distance(MovCorner a, MovCorner b){
